  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      TapButton(label: '+ Add', size: TapButtonSize.md, onPressed: () => setState(() => _showAdd = true)),
                    ],
                  ),
                  ...
                  if (widget.state.appointments.isEmpty)
                    ...
                    Flex(
                      ...
                      children: _spread(isWide, [
                        for (final appt in widget.state.appointments)
                          _ApptCard(
                            appt: appt,
                            onEdit: () => setState(() => _editing = appt),
                            onDelete: () => widget.state.deleteAppointment(appt.id),
                          ),
                      ]),
                    ),
                ],
        ...
        if (_showAdd || _editing != null)
          _ApptFormSheet(
            initial: _editing,
            onSave: (title, dateTime, location, assignee, notes) {
              if (_editing != null) {
                widget.state.updateAppointment(
                  _editing!.id,
                  Appointment(id: _editing!.id, title: title, dateTime: dateTime, location: location, assignee: assignee, notes: notes),
                );
...
class _ApptCard extends StatelessWidget {
  const _ApptCard({required this.appt, required this.onEdit, required this.onDelete});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: TapButton(label: '✎ Edit', variant: TapButtonVariant.outline, size: TapButtonSize.sm, onPressed: onEdit)),
              const SizedBox(width: 8),
              Expanded(child: TapButton(label: '🗑 Delete', variant: TapButtonVariant.destructive, size: TapButtonSize.sm, onPressed: onDelete)),
            ],
          ),
        ],
      ),
